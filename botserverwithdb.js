...
con.query('select * from flightinfo',function(err,rows,field){
  if(!err){
    console.log(rows);
  }
  else{
...
io.on( 'connection', function ( socket ) {
  ...
  socket.on( 'apicall', function ( data ) {
    ...
    if ( data.label == "whereto" ) {
     ...
     var post={label:data.distance , msg: 'Hello Raghu'
     ,destination : data.whereto, destairport :data.toairport,
     from :data.fromwhere,fromairport :data.fromairport,date:data.startdate,
     time:data.starttime,planetype:data.planetype,planespeed:data.estimatedhrs,
     planecostperhr:data.planecostperhr,email:data.email

   }
     var query = con.query('INSERT INTO flightinfo SET ?', post,
    function(err, result) {
        console.log(result);
    });
...
/*var post={label: 'dummytext', msg: 'Hello Raghu',destination : 'data', destairport :'data'}
var query = con.query('INSERT INTO flightinfo SET ?', post,
    function(err, result) {
        console.log(result);
    });
